...
(function() {
  ...
  L.TileLayer.Cordova = L.TileLayer.extend({
    initialize: function(url, options, success_callback) {
      ...
      if (!window.requestFileSystem && this.options.debug)
        console.log('L.TileLayer.Cordova: device does not support requestFileSystem');
      if (!window.requestFileSystem)
        throw 'L.TileLayer.Cordova: device does not support requestFileSystem';
      ...
      window.requestFileSystem(
        ...
        function(error) {
          ...
          throw 'L.TileLayer.Cordova: ' +
            options.name +
            ': requestFileSystem failed with code ' +
            error.code;
        }
      );
    ...
    calculateXYZListFromPyramid: function(lat, lon, zmin, zmax) {
      ...
      for (z = zmin; z <= zmax; z++) {
        ...
        if (this.options.debug)
          console.log('Calculate pyramid: Z ' + z + ' : ' + 'Radius of ' + radius);
    ...
    downloadAndStoreTile: function(x, y, z, success_callback, error_callback) {
      var myself = this;
      var onlyFileName = [myself.options.name, z, x, y].join('-') + '.png';
      var filename = myself.dirhandle.nativeURL + '/' + onlyFileName;
      var sourceurl = myself._url_online
        .replace('{z}', z)
        .replace('{x}', x)
        .replace('{y}', y);
      ...
      this._downloadFile(sourceurl, onlyFileName, success_callback, error_callback);
    },

    _downloadFile: function(sourceurl, filename, success_callback, error_callback) {
      var myself = this;
      ...
      myself.dirhandle.getFile(
            ...
            if (oReq.status === 200) {
              ...
              fileEntry.createWriter(fileWriter => {
                fileWriter.onwriteend = function() {
                  if (myself.options.debug) console.log('Successful file write...');

                  // tile downloaded OK; set the iOS "don't back up" flag then move on
                  fileEntry.setMetadata(null, null, { 'com.apple.MobileBackup': 1 });
                  if (success_callback) success_callback();
                };

                fileWriter.onerror = function(e) {
                  var errmsg = 'Failed file write:\n' + e.toString();
                  if (myself.options.debug) console.log(errmsg);
        ...
        function(err) {
          var errmsg = 'error getting file:\n';
          if (myself.options.debug) console.log(errmsg, err);
          if (error_callback) error_callback(errmsg);
        }
      );
    },

    downloadXYZList: function(
      xyzlist,
      overwrite,
      progress_callback,
      complete_callback,
    ...
    downloadTiles: function(
      ...
      function runThisOneByIndex(tilesInfo, listIndex, x, y, index, cbprog, cbdone, cberr) {
        ...
        function doneWithIt() {
          ...
          // if the progress callback returns false (not null, not undefiend... false) then do not proceed to the next tile; this allows cancellation from the caller code
          if (cbprog) {
            var keepgoing = cbprog(listIndex, x, y, index, tilesInfo.total);
            if (keepgoing === false) {
              return;
            }
          }
          ...
          if (y < item.ymax) {
            y++;
            runThisOneByIndex(tilesInfo, listIndex, x, y, index + 1, cbprog, cbdone, cberr);
          } else {
            if (x < item.xmax) {
              y = item.ymin;
              x++;
      ...
      runThisOneByIndex(
        tilesInfo,
        tilesInfo.listIndex || 0,
        tilesInfo.x || tilesInfo.list[0].xmin,
        tilesInfo.y || tilesInfo.list[0].ymin,
        tilesInfo.index || 0,
        progress_callback,
        complete_callback,
        error_callback
    ...
    // add by gisxiaowei, 2018-06-05
    deleteTiles: function(tilesInfo, progress_callback, complete_callback, error_callback) {
      var myself = this;
      ...
      function runThisOneByIndex(tilesInfo, listIndex, x, y, index, cbprog, cbdone, cberr) {
        ...
        function doneWithIt() {
          ...
          // if the progress callback returns false (not null, not undefiend... false) then do not proceed to the next tile; this allows cancellation from the caller code
          if (cbprog) {
            cbprog(listIndex, x, y, index, tilesInfo.total);
          }
        ...
        function yesReally() {
          ...
          setTimeout(
            () =>
              myself.deleteTile(x, y, z, doneWithIt, function(errmsg) {
                // an error in downloading, so we bail on the whole process and run the error callback
                if (cberr) cberr(errmsg);
              }),
            100
    ...
    // add by gisxiaowei, 2018-06-05
    deleteTile: function(x, y, z, success_callback, error_callback) {
      var myself = this;
      var onlyFileName = [myself.options.name, z, x, y].join('-') + '.png';
      var filename = myself.dirhandle.nativeURL + '/' + onlyFileName;

      if (myself.options.debug) console.log('Delete ' + filename);
      myself.dirhandle.getFile(
        onlyFileName,
        {
          create: false,
        ...
        function(fileEntry) {
          fileEntry.remove(
            function() {
              if (myself.options.debug) console.log('Delete success');
              if (success_callback) success_callback();
            },
            function(err) {

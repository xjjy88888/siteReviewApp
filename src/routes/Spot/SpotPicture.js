...
import { List, NavBar, Icon, Modal, Button, Toast } from 'antd-mobile';
...
@createForm()
export default class SpotPicture extends PureComponent {
  state = {
    photoIndex: 0,
    isOpen: false,
  };
  ...
  render() {
    const {
      spot: { imageInfos, selectedId, editable },
    } = this.props;
        ...
        <NavContentContainer>
          <PanelContentContainer>
            <List>
              ...
              {editable ? (
                <Button type="primary" onClick={() => this.imageSync(selectedId)}>
                  照片同步
                </Button>
              ) : null}
            </List>

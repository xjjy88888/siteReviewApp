...
export default class DownloadingItem extends PureComponent {
  ...
  renderItem = (item, expand, onClick, onButtonClick) => {
    ...
    // 状态
    const statusClassName = 'status-downloading';
    const statusText = `正在下载${item.percent}%`;
    const xyzObj = xyzToObjects(item.xyz);
    const downloadInfo = `切片级别:${xyzObj.listIndex} 行号:${xyzObj.x} 列号:${xyzObj.y}`;
